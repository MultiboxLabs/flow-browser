...
/**
 * Validates a custom search template and ensures it contains a fully-qualified
 * HTTP(S) URL plus the required query token.
 */
...
export function getValidCustomSearchUrlTemplateOrDefault(template: string): string {
  const validation = validateCustomSearchUrlTemplate(template);
  return validation.valid ? template.trim() : CUSTOM_SEARCH_TEMPLATE_EXAMPLE;
}
